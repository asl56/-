                ...
                <el-table :data="tableData" style="width: 90%;border-radius: 5px;margin: 0 auto;">
                    ...
                    <el-table-column prop="status" label="状态" width="150">
                        <template slot-scope="scope">
                            ...
                            <el-tag type="danger" v-else-if="scope.row.status == '未通过'">{{ scope.row.status }}</el-tag>
                    ...
                    <el-table-column label="操作" fixed="right" width="250px">
                        <template slot-scope="scope">
                            <el-popconfirm confirm-button-text='确定' cancel-button-text='取消' icon="el-icon-info"
                                icon-color="red" title="确定要删除该条招领信息吗？" @confirm="handleDelete(scope.row)">
                            ...
                            <el-link :underline="false" icon="el-icon-refresh-right" type="primary" slot="reference"
                                v-if="scope.row.status == '未通过'" @click="handleRefresh(scope.row)">重新发布</el-link>
                        </template>
                    </el-table-column>
                </el-table>
                <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
                    :current-page="currentPage" :page-sizes="[5, 10, 15, 20]" :page-size="page.count"
                    layout="total, sizes, prev, pager, next, jumper" :total="total" class="MyFound_page">
                </el-pagination>
            </div>
            ...
            <el-form :model="updateForm" :rules="rules" ref="updateForm" style="margin: 0;">
                ...
                <el-form-item label="状态" :label-width="formLabelWidth" prop="statusID"
                    ...
                    <el-select v-model="updateForm.statusID" placeholder="请选择" style="width: 340px;">
                        <el-option v-for="(item, index) in statusList" :key="index" :label="item.status"
                            :value="item.id">
                        </el-option>
                    </el-select>
                </el-form-item>
...
export default {
    ...
    methods: {
        ...
        getData() {//初始化数据
            axios.get("/main/getFound", { params: { page: this.page.page, count: this.page.count, title: this.formInline.title, releaseDate: this.formInline.date, userID: this.id } }).then(res => {
